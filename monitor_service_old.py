...
class PrometheusInit:
    def set_up_prometheus():
        start_http_server(8000)

    def process_request(status, method, endpoint):
        """Simulate processing a request and increment the counter with tuple info."""
       

    def send_metrices(sq_instance, i_table):
        sq_conn = sq_instance.connect()
        rows = fetchall(sq_conn, i_table)
        if len(rows):
            PrometheusInit.process_request()
        else:
